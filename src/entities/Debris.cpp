...
void Debris::logic(const int motion, Robot *robot) {
  ...
  // Collide with robot
  if(!isDead && !robot -> isInvincible() && collision(x, x + width, robot -> getX() + collisionBuffer, robot -> getX() + robot -> getWidth() - collisionBuffer, y, y + height, robot -> getY() + collisionBuffer, robot -> getY() + robot -> getHeight() - collisionBuffer) && !isDead) {
    // Hurt robot
    ...
    if(settings[SETTING_PARTICLE_TYPE] != 3) {
      // Sample a pixel
      ALLEGRO_COLOR sample_color = al_get_pixel(sprite, al_get_bitmap_width(sprite) / 2, al_get_bitmap_height(sprite) / 2);
