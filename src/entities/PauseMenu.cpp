...
PauseMenu::PauseMenu(afk::Scene& scene) : GameObject(scene, 0, 0, 10) {
  // Background
  scene.add<afk::Sprite>(scene, "pauseMenu", 130, 140, z).setParent(id);

  // Stats
  auto& lbl_stat_1 = scene.add<afk::Label>(scene, 220, 250, z + 1);
  lbl_stat_1.setFont("orbitron_18");
  lbl_stat_1.setParent(id);
  lbl_stat_1_id = lbl_stat_1.id;

  auto& lbl_stat_2 = scene.add<afk::Label>(scene, 220, 280, z + 1);
  lbl_stat_2.setFont("orbitron_18");
  lbl_stat_2.setParent(id);
  lbl_stat_2_id = lbl_stat_2.id;

  auto& lbl_stat_3 = scene.add<afk::Label>(scene, 220, 310, z + 1);
  lbl_stat_3.setFont("orbitron_18");
  lbl_stat_3.setParent(id);
  lbl_stat_3_id = lbl_stat_3.id;

  auto& lbl_stat_4 = scene.add<afk::Label>(scene, 220, 340, z + 1);
  lbl_stat_4.setFont("orbitron_18");
  lbl_stat_4.setParent(id);
  lbl_stat_4_id = lbl_stat_4.id;

  // Buttons
  auto& button_quit = scene.add<afk::Button>(scene, 220, 445, z + 1);
  ...
  auto& button_menu = scene.add<afk::Button>(scene, 300, 445, z + 1);
