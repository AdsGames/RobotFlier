...
void menu::draw() {
  ...
  if(mini_screen == MINISTATE_SCORES) {
    ...
    for(int i = 0; i < 10; i++) {
      al_draw_text(orbitron_24, al_map_rgb(0, 0, 0), 225, (i * 40) + 130, ALLEGRO_ALIGN_LEFT, std::to_string(highscores.getScore(i)).c_str());
      al_draw_text(orbitron_18, al_map_rgb(0, 0, 0), 575, (i * 40) + 132, ALLEGRO_ALIGN_RIGHT, highscores.getName(i).c_str());
    }
  }
  // Tutorial screen
