  ...
  if(!paused) {
    ...
    if(hectar.isAlive() && hectar.isKeyPressed()) {
      ...
      if((score >= 100 && random(0, 50) == 0) || (settings[SETTING_MEGA] && random(0, 20))) {
        Debris newAsteroid(asteroidImage, sound_asteroid, SCREEN_W, random(30, 400), 5, 1.0f, 0.0f, random(4, 20));
        debries.push_back(newAsteroid);
      }

      // Bomb spawning
      if((score >= 200 && random(0, 80) == 0) || (settings[SETTING_MEGA] && random(0, 20))) {
        Debris newBomb(bombImage, sound_bomb, SCREEN_W, random(30, 550), 10, 1.0f, 0.01f);
        debries.push_back(newBomb);
      }

      // Comets spawning
      if((score >= 300 && random(0, 200) == 0) || (settings[SETTING_MEGA] && random(0, 20))) {
        Debris newComet(cometImage, sound_asteroid, SCREEN_W, random(30, 550), 5, 1.4f, 0.01f);
...
void game::draw() {
  ...
  if(settings[SETTING_DEBUG]) {
    ...
    // Column 3
    al_draw_textf(orbitron_12, al_map_rgb(255, 255, 255), 245, 25, ALLEGRO_ALIGN_LEFT, "LowScore:%i", highscores.getScore(9));
    ...
    al_draw_textf(orbitron_12, al_map_rgb(255, 255, 255), 360, 35, ALLEGRO_ALIGN_LEFT, "Has highscore:%i", score > highscores.getScore(9));
